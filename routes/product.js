  ...
  try {
    ...
    if (itemIndex !== -1) {
      ...
      if (requestedQuantity > productStock) {
        return res
          .status(400)
          .json({ message: `Only ${productStock} items are available` });
      }
...
router.get("/api/cart/items", requirelogin, async (req, res) => {
  try {
    ...
    // Find user's cart data and populate product information
    ...
    cart.items = cart.items.filter((item) => {
      return item.product !== null && !item.product.deleted;
    });
...
//delete the product by seler end
